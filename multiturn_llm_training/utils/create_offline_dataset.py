...
class GenerationResult:
    ...
    generated_tokens_agent: List[int]  # Tokens generated by the base model
    generated_tokens_opp: List[int]  # Tokens generated by the opponent model
...
class Sample:
    ...
    chosen_reward: float
    reject_reward: float
    generated_tokens_agent: int  # Total tokens generated by base model (chosen + rejected)
    generated_tokens_opp: int  # Total tokens generated by opponent model (chosen + rejected)
    discarded_tokens: int
    sampled_h: Optional[int] = None  # REFUEL-specific, None for DPO
    discarded_tokens: bool
...
def calculate_rewards(reward_functions: List[Any], item: Dict[str, Any], conversations: List[str]) -> List[float]:
    ...
    # Process reward for each conversation
...
def get_idx_of_chosen_and_reject(rewards: List[float]) -> Tuple[int, int]:
    """Get the index of the best and worst rewards.
    
    When rewards are tied, randomly selects different indices to ensure
    best_idx != worst_idx.
    """
    best_idx = int(np.argmax(rewards))
    worst_idx = int(np.argmin(rewards))
    
    # If all rewards are the same, argmax and argmin both return 0
    # Ensure we pick different indices by randomly selecting from available indices
    if best_idx == worst_idx:
        all_indices = list(range(len(rewards)))
        best_idx, worst_idx = np.random.choice(all_indices, size=2, replace=False)
        best_idx, worst_idx = int(best_idx), int(worst_idx)
...
def create_sample(item: Dict[str, Any], generation_result: GenerationResult, 
                  rewards: List[float], chosen_reject_idx: Tuple[int, int], 
    ...
    chosen_idx, reject_idx = chosen_reject_idx

    #List of all idx that are not chosen_idx and reject_idx
    all_idx = list(range(len(generation_result.conversations)))
    all_idx.remove(chosen_idx)
    all_idx.remove(reject_idx)

    discarded_tokens = 0
    for idx in all_idx:
        discarded_tokens += generation_result.generated_tokens_agent[idx] + generation_result.generated_tokens_opp[idx]

    # Calculate total base and opp tokens for chosen + rejected conversations
    generated_tokens_agent = (generation_result.generated_tokens_agent[chosen_idx] + 
                             generation_result.generated_tokens_agent[reject_idx])
    generated_tokens_opp = (generation_result.generated_tokens_opp[chosen_idx] + 
                            generation_result.generated_tokens_opp[reject_idx])
    ...
    sample = Sample(
        ...
        generated_tokens_agent=generated_tokens_agent,
        generated_tokens_opp=generated_tokens_opp,
    ...
    if len(all_idx) >= 2:
        for i in range(len(all_idx) // 2):
            # Get rewards for remaining indices and find best/worst within that subset
            subset_rewards = [rewards[idx] for idx in all_idx]
            subset_chosen_idx, subset_reject_idx = get_idx_of_chosen_and_reject(subset_rewards)
            
            # Map subset indices back to original indices
            new_chosen_idx = all_idx[subset_chosen_idx]
            new_reject_idx = all_idx[subset_reject_idx]
            
            # Remove the original indices from all_idx
            all_idx.remove(new_chosen_idx)
            all_idx.remove(new_reject_idx)

            # Calculate total base and opp tokens for this discarded pair
            discarded_pair_tokens_base = (generation_result.generated_tokens_agent[new_chosen_idx] + 
                                          generation_result.generated_tokens_agent[new_reject_idx])
            discarded_pair_tokens_opp = (generation_result.generated_tokens_opp[new_chosen_idx] + 
                                         generation_result.generated_tokens_opp[new_reject_idx])
            ...
            discarded_samples.append(Sample(
                ...
                starting_agent=is_starting_agent,
                negotiation_role=item.get("negotiation_role", ""),
                chosen=generation_result.conversations[new_chosen_idx],
                reject=generation_result.conversations[new_reject_idx],
                chosen_token=generation_result.token_ids[new_chosen_idx],
                reject_token=generation_result.token_ids[new_reject_idx],
                chosen_mask=generation_result.assistant_masks[new_chosen_idx],
                reject_mask=generation_result.assistant_masks[new_reject_idx],
                chosen_reward=rewards[new_chosen_idx],
                reject_reward=rewards[new_reject_idx],
                generated_tokens_agent=discarded_pair_tokens_base,
                generated_tokens_opp=discarded_pair_tokens_opp,
                discarded_tokens=0,
                sampled_h=generation_result.sampled_h  # Will be None for DPO, int for REFUEL
            ))


    return sample, discarded_samples
...
def process_sample(
    ...
    num_tries: int = 0
) -> Optional[Tuple[Sample, List[Sample]]]:
    ...
    Returns:
        Tuple of (Sample, List of discarded samples) or None if failed after max retries
    """
    ...
    try:
        ...
        chosen_reject_idx = get_idx_of_chosen_and_reject(rewards)

        
        # Create sample (rewards are different)
        sample, discarded_samples = create_sample(item, generation_result, rewards, chosen_reject_idx, is_starting_agent)
...
def process_dataset(
    ...
    for i, item in enumerate(tqdm.tqdm(dataset, desc=f"{'Starting Agent' if is_starting_agent else 'Responder'} Samples")):
        ...
        # process_sample returns None on failure, or (sample, discarded_samples) on success
        if result is None:
            print(f"Skipping sample {i + 1}")
            continue
            
        sample, discarded_samples = result
        
        if sample:
            samples.append(sample)
        if discarded_samples:
            all_discarded_samples.extend(discarded_samples)

        if (i + 1) % config.progress_report_interval == 0:
            # Calculate total tokens from samples if needed for reporting
            total_tokens = sum(s.generated_tokens_agent + s.generated_tokens_opp for s in samples)
            print(f"Processed {len(samples)} samples. Total tokens: {total_tokens}")
    
    return samples, all_discarded_samples
...
def convert_to_dict_format(samples: List[Sample], args) -> Dict[str, Any]:
    ...
    result.update({
        ...
        "chosen_reward": [float(s.chosen_reward) for s in samples],
        "reject_reward": [float(s.reject_reward) for s in samples],
        "generated_tokens_agent": [s.generated_tokens_agent for s in samples],
        "generated_tokens_opp": [s.generated_tokens_opp for s in samples],
        "discarded_tokens": [s.discarded_tokens for s in samples],
    })
...
def upload_to_huggingface(dict_to_save: Dict[str, Any], args, split: str = "train") -> None:
    """Upload the dataset to Hugging Face.
    
    Args:
        dict_to_save: Dictionary containing the dataset
        args: Arguments object containing model, game_type, and hf_repo
        split: Dataset split name (e.g., "train", "discarded")
    """
    ...
    try:
        ...
        print(f"[DEBUG] Calling push_to_hub with split={split}...")
        hf_dataset.push_to_hub(args.hf_repo, dataset_name, split=split, token=api_token)
        ...
        generation_settings = {
            ...
            "elapsed_time_seconds": getattr(args, 'elapsed_time_seconds', None),
        }
